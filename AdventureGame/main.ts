...
// Enimies 
const enimies : string[] = [" Sam " , "Zombie " , "Alice" , "Bob" ];
let enimiesMaxHealth  : number = 75;
const enimiesAttackDamage : number = 25;

// Player

let playerHealth  : number = 100;
const playerAttackDamage : number = 50;
let numHealthPotoins : number = 3;
let healthPotionHealAmount : number = 50;
...
while(running){
    console.log("----------------------------------------------------------------")
    let enemyHealth : number = Math.random() * enimiesMaxHealth 
    ...
    while(enemyHealth > 0) {
        ...
        if(user.action == "Attack"){
            let enemyDamage : number = Math.random() * playerAttackDamage
            let playerDamage : number = Math.random() * enimiesAttackDamage
